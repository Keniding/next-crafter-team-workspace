# Spec: manual entry (new capability)
...
- THEN Convex MUST create a `repository` per `repository-cemetery-entity`'s spec, with `origin: manual`, `whyItDied` as the public autopsy, and `evidence` mapped to `artifacts`
