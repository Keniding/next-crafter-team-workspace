# Spec: business auth (new capability)
...
`packages/user`'s prior "no second auth" rule (`workspace-module-contracts/specs/user.md`) is superseded. Clerk, bridged exclusively through `packages/auth`, is the one sanctioned identity provider.
...
- THEN that change MUST be rejected as out of scope
- AND MUST cite this requirement, not the old "no second auth" scenario
